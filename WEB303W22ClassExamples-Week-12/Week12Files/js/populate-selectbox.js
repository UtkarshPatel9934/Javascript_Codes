(function () {
  var form = document.getElementById('howHeard');
  ...
  form.addEventListener("submit", (e) => {
    e.preventDefault();
    console.log(`Equipment type value: ${type.value}, and the model value: ${model.value}`)
  });
  // WHEN THE USER CHANGES THE TYPE SELECT BOX
  type.addEventListener('change', (e) => {
    let value = e.target.value;
    if (value === 'choose') {                // No selection made
      model.innerHTML = '<option value="choose">Please choose a type first</option>';
      return;                                     // No need to proceed further
    }
    var models = getModels(value);           // Select the right object
    /* 
    // either 
    models = cameras;
    // or
    ...
    */
    ...
    var options = '<option value="choose">Please choose a model</option>';
    var key;
