...
(function () {
  ...
  $('form').on('submit', function (e) {      // When form is submitted
    ...
    if (!validatePassword()) {          // Call validatePassword(), and if not valid
      ...
      if (!validateConfirmPassword()) {
        showErrorMessage(document.getElementById('conf-password')); // Show error message
  ...
  function validateConfirmPassword() {
    ...
    // var valid = passwordsMatch(null, confPassword);
    var valid = confPassword.className != "fail"; // the faster way, relying on changes made to the page by the other javascript file
    if (!valid) {
      setErrorMessage(confPassword, 'Please make sure your passwords match');
    }
    return valid;
  }
  ...
  function showErrorMessage(el) {
    ...
    errorContainer.text(getErrorMessage(el)).show();             // Add error message
  }

  function removeErrorMessage(el) {
    var errorContainer = $(el).siblings('.error.message'); // Get the sibling of this form control used to hold the error message
    errorContainer.hide();                               // Remove the element that contains the error message
  }
